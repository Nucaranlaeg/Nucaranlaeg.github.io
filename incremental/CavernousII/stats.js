class Stat {
	...
	gainSkill(amount) {
		...
		let val = (this.current + 1) ** (0.9 * (this.current > 100 ? 100 / this.current : 1)) - (this.base + 1);
		if (val < 0) return;
		let prevVal = (prev + 1) ** (0.9 * (prev > 100 ? 100 / prev : 1)) - (this.base + 1);
	...
	update() {
		...
		if (this.name == "Mana"){
			...
			let increaseRequired = (this.base + 1) ** (0.009 * (this.base > 100 ? this.base : 100)) - 1;
