...
const runes = [
    ...
    new Rune("Wither", "H", simpleRequire([["Salt", 1], ["Iron Ore", 1], ["Gold Nugget", 1]]), 0, () => `This rune allows you to kill even hardy orthogonally ${((getRune("Wither").upgradeCount || 0) > 0 ? "or diagonally " : "")}adjacent plants.  Interact with it to charge it up - it takes ${((getRune("Wither").upgradeCount || 0) > 1 ? "1/" + 2 ** (getRune("Wither").upgradeCount - 1) : "")} as much time to charge as the plants you're trying to kill would take to chop.<br>Requires:<br>{'0':'1 Salt<br>1 Iron Ore${getRealm("Verdant Realm").completed ? "" : "<br>1 Gold Nugget"}', '1':'2 Salt<br>2 Iron Ore${getRealm("Verdant Realm").completed ? "" : "<br>2 Gold Nuggets"}'}`, null, "Charge Wither"),
